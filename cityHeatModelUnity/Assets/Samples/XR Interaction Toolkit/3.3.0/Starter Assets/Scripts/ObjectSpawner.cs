...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ObjectSpawner : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("Debug Planed Toggle.")]
        Button m_PlanesToggle;

        /// <summary>
        /// Menu Manager.
        /// </summary>
        public Button PlanesToggle
        {
            get => m_PlanesToggle;
            set => m_PlanesToggle = value;
        }
        ...
        /// <summary>
        /// The index of the prefab to spawn.
        /// </summary>
        ///
        ...
        public bool TrySpawnObject(Vector3 spawnPoint, Vector3 spawnNormal)
        {
            if(m_ObjectSpawned)
            {
                Debug.LogWarning("An object has already been spawned and further spawns are disabled.", this);
                return false;
            }
            ...
            // Refresh heatmap textures on spawned clones using reflection to avoid namespace issues
            System.Type heatmapManagerType = System.Type.GetType("ARHeatmapManager");
            if (heatmapManagerType == null)
            {
                foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    heatmapManagerType = asm.GetType("ARHeatmapManager");
                    if (heatmapManagerType != null) break;
                }
            }

            if (heatmapManagerType != null)
            {
                var heatmapManager = Object.FindObjectOfType(heatmapManagerType);
                if (heatmapManager != null)
                {
                    heatmapManagerType.GetMethod("RefreshAndApplyTexture")?.Invoke(heatmapManager, null);
                }
            }
            PlanesToggle.onClick.Invoke();
